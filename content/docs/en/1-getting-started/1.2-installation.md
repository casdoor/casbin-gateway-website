...
Running the command again over an existing installation updates it: the Gateway that is running is stopped first, so the new build is what serves afterwards, and started again once it is in place.

| Variable | Effect |
| --- | --- |
| `INSTALL_DIR` | Install somewhere other than the default directory |
| `NO_START=1` | Install without starting. A Gateway the install stopped is still started again |
