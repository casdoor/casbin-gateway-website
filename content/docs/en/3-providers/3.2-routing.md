...
For Codex it also writes a model catalog listing the provider's models, so Codex's model picker names the bound model instead of calling it **Custom**. Gateway has Codex itself read the catalog first and writes none that Codex would refuse, since a catalog it cannot parse stops Codex from starting.
